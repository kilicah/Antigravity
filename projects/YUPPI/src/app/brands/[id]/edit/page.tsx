...
export default async function EditBrandPage({
  ...
  async function updateBrand(formData: FormData) {
    ...
    // First delete exising reps for simplicity, then recreate
    await prisma.$transaction(async (tx) => {
      await tx.brandRepresentative.deleteMany({
        where: { brandId: id }
      });
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Marka Düzenle: {brand.name}</h1>
        <Link 
          href="/brands"
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
        <form action={updateBrand} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input 
                ...
                id="registrationNo" 
                name="registrationNo" 
                defaultValue={brand.registrationNo || ''}
                className="w-full px-4 py-2 border border-slate-300 rounded-md focus:ring-blue-500 focus:border-blue-500 uppercase"
              />
            </div>
          </div>
          ...
          <div className="pt-6 mt-6 border-t border-slate-200">
            <h2 className="text-xl font-semibold mb-4 text-slate-800">Marka Yetkilileri (Zorunlu Değil)</h2>
            
            {[1, 2, 3].map((index) => {
              const rep = brand.representatives[index - 1] || {};
              return (
