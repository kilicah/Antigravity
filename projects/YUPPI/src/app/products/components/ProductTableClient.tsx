  ...
  const handleTierChange = (tierName: "priceTier1" | "priceTier2" | "priceTier3", valStr: string) => {
     setFormData(prev => {
        ...
        if (autoCalc && config && targetTier === tierName) {
           ...
           if (!isNaN(val)) {
              const res = (val * config.commissionRate) + config.freightCost;
              nGbp = String(parseFloat((res / config.usdToGbpRate).toFixed(2)));
              nEur = String(parseFloat((res / config.usdToEurRate).toFixed(2)));
           } else if (valStr === "") {
              nGbp = "";
              nEur = "";
           }
  ...
  return (
    <div className="w-full h-full flex flex-col items-start relative pb-20">
        ...
        <div className="flex space-x-3">
          {userRole !== 'USER' && (
            <button 
              ...
              className="group relative inline-flex items-center justify-center px-4 py-2.5 text-sm font-bold text-slate-800 transition-all duration-200 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 shadow-sm"
            >
          ...
          {userRole !== 'USER' && (
            <button 
              ...
              className="group relative inline-flex items-center justify-center px-6 py-2.5 text-sm font-medium text-white transition-all duration-200 bg-slate-900 border border-transparent rounded-lg hover:bg-slate-800 shadow-md"
            >
      ...
      <div className="sticky top-[80px] z-10 flex w-full items-center gap-3 mb-4 bg-white/90 backdrop-blur-xl p-3 rounded-xl border border-slate-200/60 shadow-md transition-all shrink-0">
        {selectedProductIds.length > 0 && userRole !== 'USER' && (
           <div className="flex bg-indigo-50 items-center px-3 py-1.5 rounded-lg border border-indigo-200 shadow-sm space-x-2">
             <span className="text-xs font-bold text-indigo-800">{selectedProductIds.length} Seçili</span>
             ...
             {selectedProductIds.length === 1 && (
               ...
               }} className="text-[11px] font-bold text-indigo-700 bg-white border border-indigo-200 px-2.5 py-1 rounded shadow-sm hover:bg-indigo-100 transition">Düzenle</button>
             )}
             <button onClick={() => { setBulkTypes([]); setIsBulkTypeModalOpen(true); }} className="text-[11px] font-bold text-blue-700 bg-white border border-blue-200 px-2.5 py-1 rounded shadow-sm hover:bg-blue-100 transition">Cinsle</button>
             
             {activeTab !== 'active' && (
                <button onClick={() => handleBulkAction('aktifle')} className="text-[11px] font-bold text-emerald-700 bg-white border border-emerald-200 px-2.5 py-1 rounded shadow-sm hover:bg-emerald-100 transition">Aktifle</button>
             )}
             {activeTab !== 'passive' && (
                <button onClick={() => handleBulkAction('pasifle')} className="text-[11px] font-bold text-orange-700 bg-white border border-orange-200 px-2.5 py-1 rounded shadow-sm hover:bg-orange-100 transition">Pasifle</button>
             )}
             {activeTab !== 'cancelled' && (
                <button onClick={() => handleBulkAction('iptal')} className="text-[11px] font-bold text-slate-700 bg-white border border-slate-300 px-2.5 py-1 rounded shadow-sm hover:bg-slate-100 transition">İptal Et</button>
             )}
              {(activeTab === 'passive' || activeTab === 'cancelled') && userRole === 'ADMIN' && (
                 <button onClick={handleDelete} className="text-[11px] font-bold text-red-700 bg-white border border-red-200 px-2.5 py-1 rounded shadow-sm hover:bg-red-100 transition flex items-center">
        ...
        <div className="relative ml-2 w-64">
           ...
           <input
             ...
             className="block w-full pl-10 pr-3 py-2 border border-slate-200 rounded-lg bg-white/50 focus:bg-white text-sm focus:ring-2 focus:ring-slate-800 focus:border-slate-800 transition-colors placeholder:text-slate-400 text-slate-800"
           />
        </div>

        <div className="ml-auto flex flex-col md:flex-row shrink-0 border-l border-slate-200 pl-4 space-x-2">
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-bold transition-all shadow-sm border ${
              activeTab === "active" ? "bg-white text-slate-800" : "bg-transparent text-slate-500 border-transparent shadow-none hover:bg-slate-100"
            }`}
          >
            Aktif
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-bold transition-all shadow-sm border ${
              activeTab === "new" ? "bg-white text-slate-800" : "bg-transparent text-slate-500 border-transparent shadow-none hover:bg-slate-100"
            }`}
          >
            Yeni
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-bold transition-all shadow-sm border ${
              activeTab === "passive" ? "bg-white text-slate-800" : "bg-transparent text-slate-500 border-transparent shadow-none hover:bg-slate-100"
            }`}
          >
            Pasif
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-bold transition-all shadow-sm border ${
              activeTab === "cancelled" ? "bg-white text-slate-800" : "bg-transparent text-slate-500 border-transparent shadow-none hover:bg-slate-100"
            }`}
          >
            İptal
          </button>
      ...
      <div className="w-full bg-white/80 backdrop-blur-xl rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden flex flex-col">
          <div className="overflow-x-auto overflow-y-auto max-h-[calc(100vh-220px)] relative">
            <table className="w-full text-left border-collapse whitespace-nowrap min-w-[1200px]">
              <thead className="sticky top-0 z-20 bg-slate-50/95 backdrop-blur-sm shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
                <tr className="text-slate-500 text-[11px] uppercase tracking-wider font-bold select-none">
                  ...
                  <th 
                    className="py-4 px-4 font-semibold cursor-pointer hover:bg-slate-100/50 transition-colors group"
                    onClick={() => handleSort("code")}
                  >
                    <div className="flex items-center">
                      Kodu
                      {sortConfig?.key === "code" && (
                         <span className="ml-1 text-slate-800">{sortConfig.dir === "asc" ? "▲" : "▼"}</span>
                      )}
                    </div>
                  </th>
                  <th 
                    className="py-4 px-4 font-semibold w-64 cursor-pointer hover:bg-slate-100/50 transition-colors group"
                  ...
                  <th className="py-4 px-3 font-semibold text-center text-slate-600">CİNSİ</th>
                  <th 
                    className="py-4 px-3 font-semibold cursor-pointer hover:bg-slate-100/50 transition-colors group"
                  ...
                  {userRole === 'ADMIN' && <th className="py-4 px-3 font-semibold text-right">Maliyet($)</th>}
                  <th className="py-4 px-3 font-semibold text-right">T1 ($)</th>
                  <th className="py-4 px-3 font-semibold text-right">T2 ($)</th>
                  <th className="py-4 px-3 font-semibold text-right">T3 ($)</th>
                  <th className="py-4 px-3 font-semibold text-right">GBP(£)</th>
                  <th className="py-4 px-3 font-semibold text-right">EUR(€)</th>
              ...
              <tbody className="divide-y divide-slate-100/50">
                {filteredProducts.length === 0 ? (
                  ...
                  filteredProducts.map((p) => {
                    ...
                    return (
                      <tr key={p.id} onClick={() => handleCheckboxChange(p.id)} className={`transition-colors group cursor-pointer ${isSelected ? 'bg-indigo-50/50' : 'hover:bg-slate-50/80'}`}>
                        <td className="py-3 px-4 text-center">
                          <input type="checkbox" className="w-5 h-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-600 transition-all cursor-pointer" checked={isSelected} onChange={() => {}} />
                        </td>
                        <td className="py-3 px-4 text-sm font-semibold text-slate-900">
                           {p.code ? <span className="bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200">{p.code}</span> : "-"}
                        </td>
                        <td className="py-3 px-4 text-sm font-semibold text-slate-900 whitespace-normal min-w-[200px]">{p.name || "-"}</td>
                        <td className="py-3 px-3 text-center">
                           <div className="flex flex-wrap justify-center gap-1">
                              {p.productTypes && p.productTypes.split(",").map((t: string) => {
                                 ...
                                 else if(t === 'FP') { bg = "bg-emerald-700"; text = "text-white"; border = "border-emerald-800"; }
                        ...
                        <td className="py-3 px-3 text-xs whitespace-normal">
                           <div className="font-semibold text-slate-900">{p.composition || "-"}</div>
                           <div className="font-normal text-slate-900 mt-0.5">
                              {p.weight ? p.weight : "-"} GSM / {p.width ? p.width : "-"} CM
                           </div>
                        </td>
                        {userRole === 'ADMIN' && (
                           <td className="py-3 px-3 text-sm text-right font-medium text-slate-500">
                              {p.costPrice ? p.costPrice.toFixed(2) : "-"}
                           </td>
                        )}
                        <td className="py-3 px-3 text-sm text-right font-medium text-slate-900">{p.priceTier1 ? p.priceTier1.toFixed(2) : "-"}</td>
                        <td className="py-3 px-3 text-sm text-right font-medium text-slate-900">{p.priceTier2 ? p.priceTier2.toFixed(2) : "-"}</td>
                        <td className="py-3 px-3 text-sm text-right font-bold text-slate-900">{p.priceTier3 ? p.priceTier3.toFixed(2) : "-"}</td>
                        <td className="py-3 px-3 text-sm text-right font-bold text-blue-700">{p.poundPrice ? p.poundPrice.toFixed(2) : "-"}</td>
                        <td className="py-3 px-3 text-sm text-right font-bold text-rose-900">{p.euroPrice ? p.euroPrice.toFixed(2) : "-"}</td>
                        <td className="py-3 px-3 text-sm text-center font-bold text-indigo-700 bg-indigo-50/30">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden border border-slate-200 flex flex-col max-h-[90vh]">
             <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center shrink-0">
               <h3 className="text-xl font-bold text-slate-800">{editProduct ? "Ürün Düzenle" : "Yeni Ürün Ekle"}</h3>
             ...
             <div className="overflow-y-auto p-6">
               <form id="productForm" onSubmit={handleSubmit}>
                  ...
                  <div className="flex justify-between items-center border-b border-indigo-100 pb-2 mb-4">
                     <h4 className="text-sm font-bold text-indigo-700 uppercase tracking-widest">Temel Bilgiler</h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                     <div>
                        ...
                        <input required type="text" value={formData.code} onChange={e => setFormData({...formData, code: e.target.value})} className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-slate-800 focus:border-slate-800 transition-all font-mono" />
                     </div>
                     <div>
                        <label className="block text-[12px] font-bold text-slate-500 mb-1.5 uppercase tracking-wide">Adı / Kalitesi *</label>
                        <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-slate-800 focus:border-slate-800 transition-all font-bold" />
                     </div>
                  </div>
                     ...
                     <div className="flex flex-wrap gap-3">
                        {[
                           ...
                           <label key={type.val} className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg cursor-pointer transition border ${formData.productTypes.includes(type.val) ? 'bg-indigo-50 border-indigo-200' : 'bg-slate-50 border-slate-200 hover:bg-slate-100'}`}>
                              ...
                              />
                              <span className={`text-xs font-bold ${formData.productTypes.includes(type.val) ? 'text-indigo-800' : 'text-slate-600'}`}>{type.label} ({type.val})</span>
                           </label>
                        ))}
                     </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center border-b border-blue-100 pb-2 mb-4">
                     <h4 className="text-sm font-bold text-blue-700 uppercase tracking-widest">ÜR-GE (Özellikler)</h4>
                     <button type="button" onClick={() => setIsRndModalOpen(true)} className="text-xs bg-blue-50 text-blue-700 px-3 py-1.5 rounded border border-blue-200 hover:bg-blue-100 transition shadow-sm font-semibold flex items-center">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
                     <div className="md:col-span-3">
                        ...
                        <input type="text" value={formData.composition} onChange={e => setFormData({...formData, composition: e.target.value})} placeholder="%100 Pamuk vb." className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-slate-800 transition-all" />
                     </div>
                     <div>
                        <label className="block text-[12px] font-bold text-slate-500 mb-1.5 uppercase tracking-wide">Ağırlık</label>
                        <input type="text" value={formData.weight} onChange={e => setFormData({...formData, weight: e.target.value})} placeholder="150 gr/m2" className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-slate-800 transition-all" />
                     </div>
                     <div>
                        <label className="block text-[12px] font-bold text-slate-500 mb-1.5 uppercase tracking-wide">En</label>
                        <input type="text" value={formData.width} onChange={e => setFormData({...formData, width: e.target.value})} placeholder="150 cm" className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-slate-800 transition-all" />
                     </div>
                     <div>
                        <label className="block text-[12px] font-bold text-slate-500 mb-1.5 uppercase tracking-wide">GTİP No</label>
                        <input type="text" value={formData.gtipNo} onChange={e => setFormData({...formData, gtipNo: e.target.value})} className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-slate-800 transition-all font-mono" />
                     </div>
                  </div>
                  ...
                  <h4 className="text-sm font-bold text-emerald-700 uppercase tracking-widest border-b border-emerald-100 pb-2 mb-4 flex justify-between items-center">
                     ...
                     <label className="flex items-center text-xs font-medium text-slate-600 bg-white border border-slate-200 px-2 py-1 rounded cursor-pointer hover:bg-slate-50 transition">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 mb-5">
                     <div className="bg-rose-50/50 p-2 rounded-lg border border-rose-100">
                        <label className="block text-[11px] font-bold text-rose-700 mb-1.5 uppercase tracking-wide">Maliyet ($)</label>
                        <input type="number" step="0.01" value={formData.costPrice} onChange={e => setFormData({...formData, costPrice: e.target.value})} className="w-full bg-white border border-rose-200 text-slate-900 rounded px-2 py-1.5 outline-none focus:ring-2 focus:ring-rose-500 transition-all font-mono text-sm" />
                     </div>
                     <div>
                        ...
                        <input type="number" step="0.01" value={formData.priceTier1} onChange={e => handleTierChange("priceTier1", e.target.value)} className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded px-2 py-1.5 outline-none focus:ring-2 focus:ring-emerald-500 transition-all font-mono text-sm" />
                     </div>
                     <div>
                        ...
                        <input type="number" step="0.01" value={formData.priceTier2} onChange={e => handleTierChange("priceTier2", e.target.value)} className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded px-2 py-1.5 outline-none focus:ring-2 focus:ring-emerald-500 transition-all font-mono text-sm" />
                     </div>
                     <div>
                        ...
                        <input type="number" step="0.01" value={formData.priceTier3} onChange={e => handleTierChange("priceTier3", e.target.value)} className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded px-2 py-1.5 outline-none focus:ring-2 focus:ring-emerald-500 transition-all font-mono text-sm" />
                     </div>
                     <div>
                        ...
                        <input type="number" step="0.001" value={formData.poundPrice} onChange={e => setFormData({...formData, poundPrice: e.target.value})} className={`w-full bg-slate-50 border border-slate-200 text-slate-900 rounded px-2 py-1.5 outline-none focus:ring-2 focus:ring-blue-500 transition-all font-mono text-sm ${autoCalc ? 'bg-blue-50/50' : ''}`} />
                     </div>
                     <div>
                        ...
                        <input type="number" step="0.001" value={formData.euroPrice} onChange={e => setFormData({...formData, euroPrice: e.target.value})} className={`w-full bg-slate-50 border border-slate-200 text-slate-900 rounded px-2 py-1.5 outline-none focus:ring-2 focus:ring-blue-500 transition-all font-mono text-sm ${autoCalc ? 'bg-blue-50/50' : ''}`} />
                     </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-5 mb-6">
                     <div>
                        ...
                        <input type="number" step="0.01" value={formData.tMoq} onChange={e => setFormData({...formData, tMoq: parseFloat(e.target.value) || 0})} className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-slate-800 transition-all" />
                     </div>
                     <div>
                        <label className="block text-[12px] font-bold text-slate-500 mb-1.5 uppercase tracking-wide">Renk MOQ (MCQ)</label>
                        <input type="number" step="0.01" value={formData.mcq} onChange={e => setFormData({...formData, mcq: parseFloat(e.target.value) || 0})} className="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-slate-800 transition-all" />
                     </div>
                     <div>
             ...
             <div className="flex justify-end gap-3 p-5 border-t border-slate-200 bg-slate-50 shrink-0">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 py-2.5 rounded-lg border border-slate-300 text-slate-700 font-bold hover:bg-slate-100 transition-colors">Kapat</button>
             ...
             <div className="p-6">
                ...
                <form onSubmit={handleImportSubmit} className="space-y-4">
                  ...
                  <div className="pt-2">
                      ...
                      <input type="text" value={importNote} onChange={e => setImportNote(e.target.value)} placeholder="Excel içeriği için genel bir not (Yüklenme tarihi vb.)" className="w-full bg-white border border-slate-200 text-slate-900 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500 transition-all text-sm" />
                   </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 flex flex-col">
             <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
               ...
               <button onClick={() => setIsRndModalOpen(false)} className="text-slate-400 hover:text-slate-600">
             ...
             <div className="p-6">
                <input 
                  ...
                  className="w-full bg-white border-2 border-slate-200 text-slate-900 rounded-lg px-4 py-3 outline-none focus:ring-0 focus:border-blue-500 transition-colors font-medium mb-4" 
                />
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 flex flex-col">
             <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
               ...
               <button onClick={() => setIsBulkTypeModalOpen(false)} className="text-slate-400 hover:text-slate-600">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
               </button>
             </div>
             <div className="p-6">
                        ...
                        {[
                           ...
                           <label key={type.val} className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg cursor-pointer transition border ${bulkTypes.includes(type.val) ? 'bg-indigo-50 border-indigo-200' : 'bg-slate-50 border-slate-200 hover:bg-slate-100'}`}>
                              <input 
                                 ...
                                 className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-600 w-4 h-4"
                              />
                              <span className={`text-xs font-bold ${bulkTypes.includes(type.val) ? 'text-indigo-800' : 'text-slate-600'}`}>{type.label} ({type.val})</span>
                           </label>
                        ))}
                     </div>
                </div>
