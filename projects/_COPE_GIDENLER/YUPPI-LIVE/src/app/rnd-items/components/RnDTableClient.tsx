...
export default function RnDTableClient({ items }: { items: any[] }) {
    ...
    <div className="w-full h-full flex flex-col items-start relative pb-20">
      <div className="sticky top-0 z-20 w-full bg-slate-50/95 backdrop-blur-md shadow-sm border-b border-slate-200 flex justify-between items-center pb-4 mb-3 pt-2 px-2">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 font-medium">Bağımsız Kolleksiyon / ÜR-GE Deposu</p>
        </div>
      </div>
      ...
      <div className="w-full h-auto bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden flex flex-col mt-2">
            ...
            <table className="w-full text-left border-collapse table-auto min-w-max">
              <thead>
                <tr className="bg-slate-100 text-slate-600 text-[11px] uppercase tracking-wider font-extrabold border-b border-slate-300">
                  <th className="py-4 px-4 sticky top-0 bg-slate-100 z-10 w-24">Kodu</th>
                  <th className="py-4 px-3 sticky top-0 bg-slate-100 z-10 w-64">Kalite Adı</th>
                  <th className="py-4 px-3 sticky top-0 bg-slate-100 z-10">Konstrüksiyon</th>
                  <th className="py-4 px-4 sticky top-0 bg-slate-100 z-10 text-right w-24">İşlemler</th>
                </tr>
              </thead>
                      ...
                      <React.Fragment key={p.id}>
                        <tr className="hover:bg-blue-50/50 transition-colors group">
                          ...
                          <td className="py-3 px-4 text-right">
                             {rawDataObj && (
                                <button onClick={() => toggleRow(p.id)} className="text-xs bg-white text-slate-700 px-3 py-1.5 rounded-lg border border-slate-300 hover:bg-slate-100 hover:text-blue-700 transition-all font-bold shadow-sm whitespace-nowrap flex items-center justify-end ml-auto">
                        ...
                        {isExpanded && rawDataObj && (
                          <tr className="bg-slate-50/80 border-b border-slate-200">
                             <td colSpan={4} className="py-4 px-6">
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 bg-white rounded-xl border border-slate-200 shadow-inner">
                                   {Object.entries(rawDataObj).map(([key, val]) => {
                                      if(!val || val === "") return null;
                                      return (
                                        <div key={key} className="flex flex-col border-b border-slate-100 pb-2">
                                           <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-1">{key}</span>
