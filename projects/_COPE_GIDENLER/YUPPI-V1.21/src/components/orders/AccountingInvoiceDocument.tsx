      ...
      <div className="w-[1002px] bg-white text-black font-['Arial',_'Helvetica',_sans-serif] text-[12px] leading-tight border-2 border-black relative">
        ...
        <div className="grid grid-cols-[401px_401px_200px] border-b-2 border-black">
          ...
          <div className="border-r-2 border-black p-2 flex justify-center items-center h-[135px]">
              <h1 className="text-[#0e7490] font-sans font-bold text-[36px] text-center leading-tight tracking-wider uppercase">
                {isEng ? (
        ...
        <div className="grid grid-cols-[580px_1fr] border-b-2 border-black">
           ...
           <div className="flex flex-col">
              <div className="grid grid-cols-[130px_1fr] border-b border-black">
                 <div className="p-1 font-bold uppercase border-r border-black">{t.agencyName}</div>
                 <div className="p-1 uppercase text-[12px] leading-tight flex items-center">{order.agency?.name || "-"}</div>
              </div>
              <div className="grid grid-cols-[130px_1fr] border-b border-black">
                 <div className="p-1 font-bold uppercase border-r border-black flex items-center">A. ÜNVANI / V.D.</div>
                 <div className="p-1 uppercase text-[10px] leading-tight flex items-center">{order.agency?.taxOffice ? order.agency.taxOffice + " VD. / " + (order.agency.taxNo || "-") : "-"}</div>
              </div>
              <div className="grid grid-cols-[130px_1fr] border-b border-black">
                 <div className="p-1 font-bold uppercase border-r border-black flex items-center">TEMSİLCİ</div>
                 <div className="p-1 uppercase text-[12px] leading-tight flex items-center">
                    {order.sellerRep || "-"} {order.sellerRep && order.buyerRep ? `/ ${order.buyerRep}` : (order.buyerRep || "")}
                 </div>
              </div>
              <div className="grid grid-cols-[130px_1fr] border-b border-black">
                 <div className="p-1 font-bold uppercase border-r border-black">{t.commission}</div>
                 <div className="p-1 uppercase text-[12px] leading-tight flex flex-col justify-center">
                    <div className="font-mono font-bold text-[#d81e1e]">{order.commission || "-"}</div>
                    {order.commission && <div className="text-[9px] text-slate-500 normal-case mt-0.5">Bu sipariş özelinde acenta hak edişidir.</div>}
                 </div>
              </div>
              
              <div className="flex-1 bg-white min-h-[50px] border-b border-black"></div>
