...
export default function AccountingInvoiceDocument({ order, bankInfo, repUser }: { order: any; bankInfo: any; repUser?: any }) {
  ...
  const t = {
    ...
    agencyName: isEng ? "AGENCY INFO:" : "ACENTA BİLGİSİ:",
    commission: isEng ? "COMMISSION RATE:" : "KOMİSYON (S.B):",
    repName: isEng ? "REPRESENTATIVE:" : "TEMSİLCİ:",
      ...
      <div className="w-[1002px] bg-white text-black font-['Arial',_'Helvetica',_sans-serif] text-[12px] leading-tight border-2 border-black relative">
        ...
        <div className="grid grid-cols-[401px_401px_200px] border-b-2 border-black">
          ...
          <div className="flex flex-col h-[135px]">
              ...
              <div className="h-[45px] border-b border-black flex flex-col justify-center px-4 leading-snug">
                  ...
                  <div className="text-[12px]">{order.contractNo || "-"}</div>
              </div>
              
              {/* Tarihi (45px) */}
              <div className="h-[45px] border-b border-black flex flex-col justify-center px-4 leading-snug">
                  <div className="font-bold uppercase text-[13px] mb-0.5">{t.invoiceDate}</div>
                  <div className="text-[12px]">{order.contractDate ? new Date(order.contractDate).toLocaleDateString('tr-TR') : "-"}</div>
              </div>
        ...
        <div className="flex flex-col border-t-2 border-b-2 border-black">
          ...
          <div className="grid grid-cols-[580px_1fr]">
            {/* Left details */}
            <div className="border-r-2 border-black flex flex-col justify-start">
            ...
            {/* Right Agency & Commission */}
            <div className="flex flex-col justify-start pb-1">
              <div className="grid grid-cols-[130px_1fr] border-b border-black">
                 <div className="p-1 font-bold uppercase border-r border-black flex items-center">{t.agencyName}</div>
                 <div className="p-1 uppercase text-[12px] leading-tight flex items-center">{order.agency?.name || "-"}</div>
              </div>
              <div className="grid grid-cols-[130px_1fr] border-b border-black">
                 <div className="p-1 font-bold uppercase border-r border-black flex items-center">{t.commission}</div>
                 <div className="p-1 uppercase text-[12px] leading-tight flex items-center font-mono font-bold text-[#d81e1e]">
                    {order.commission || "-"}
                 </div>
              </div>
              <div className="grid grid-cols-[130px_1fr]">
                 <div className="p-1 font-bold uppercase border-r border-black flex items-center">{t.repName}</div>
                 <div className="p-1 uppercase text-[12px] leading-tight flex items-center">
                    {order.sellerRep || "-"}
                 </div>
              </div>
            </div>
          ...
          <div className="border-t-2 border-black flex flex-col p-2 min-h-[60px] bg-white text-[12px]">
             {order.productionOrder?.packingInstructions && (
                <div className="mb-2 flex">
                   <span className="font-bold uppercase underline w-[150px] shrink-0 mr-2">{isEng ? 'ORDER NOTES:' : 'SİPARİŞ NOTLARI:'}</span>
                   <span className="uppercase whitespace-pre-wrap">{order.productionOrder.packingInstructions}</span>
                </div>
             )}
             {order.accountingNotes && (
                <div className="flex mt-1">
                   <span className="font-bold uppercase underline w-[150px] shrink-0 mr-2">{isEng ? 'ACCOUNTING NOTES:' : 'MUHASEBE NOTLARI:'}</span>
                   <span className="uppercase whitespace-pre-wrap">{order.accountingNotes}</span>
                </div>
             )}
             {(!order.productionOrder?.packingInstructions && !order.accountingNotes) && (
                <div className="text-transparent min-h-[30px]">No notes</div>
             )}
          </div>
